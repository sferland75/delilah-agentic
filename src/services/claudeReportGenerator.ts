type AssessmentData = any; // We'll define proper types later
type ReportSection = string;

function getSectionPrompt(sectionType: string, data: any): string {
    const prompts = {
...
async function generateReportSection(sectionType: string, data: AssessmentData): Promise<ReportSection> {
