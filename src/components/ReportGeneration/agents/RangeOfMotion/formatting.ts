...
function formatBrief(data: ROMAnalysis): string {
  ...
  if (data.patterns.unilateral.length > 0) {
    findings.push(...data.patterns.unilateral.map(p => 
      `Asymmetrical ${p.joint} ${p.movement} (${p.difference}° difference)`
    ));
  }

  if (data.patterns.restricted.length > 0) {
    findings.push(...data.patterns.restricted.map(p => 
      `${p.joint} ${p.movement} restricted`
    ));
  }

  if (data.patterns.painful.length > 0) {
    findings.push(...data.patterns.painful.map(p => 
      `${p.joint} ${p.movement} painful`
    ));
  }
...
export { formatBrief, formatStandard, formatDetailed };