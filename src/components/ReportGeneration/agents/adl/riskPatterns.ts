...
import { IADLRiskPatterns } from './types';

export const RISK_PATTERNS: IADLRiskPatterns = {
  transportation: [
    {
...
export function identifyRisks(
  activity: string,
  details: { notes: string; independence: IndependenceLevel }
): ProcessedIADLData['safetyConsiderations'][0] | null {
  const notesLower = details.notes.toLowerCase();
  
  // Check if any patterns match for this activity
  const activityPatterns = RISK_PATTERNS[activity] || [];
