...
interface AMAGuidesAssessmentProps {
  assessment: Assessment;
}

export const AMAGuidesAssessment: React.FC<AMAGuidesAssessmentProps> = ({ assessment }) => {
  const { amaGuides } = assessment;

  const renderRating = (value: number) => {
    if (value === 0) return 'No Impairment';
    if (value === 1) return 'Mild Impairment';
    if (value === 2) return 'Moderate Impairment';
    if (value === 3) return 'Severe Impairment';
    if (value === 4) return 'Profound Impairment';
    return 'Not Assessed';
  };

  const calculateImpairmentClass = (value: number) => {
  ...
  return (
    <div>
      ...
      {(amaGuides.overallRating !== undefined || amaGuides.generalNotes) && (
        <div className="mt-6">
          {amaGuides.overallRating !== undefined && (
            <div className={`font-bold mb-3 ${calculateImpairmentClass(amaGuides.overallRating)}`}>
              Overall Rating: {renderRating(amaGuides.overallRating)}
            </div>
          )}
          {amaGuides.generalNotes && (
            <div className="mt-4">
