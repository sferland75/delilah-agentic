...
import { DEFAULT_TRANSFER_VALUES } from './transfers-default-values';
...
export function TransfersAssessment() {
  ...
  useEffect(() => {
    ...
    const currentValues = getValues('functionalAssessment.transfers');
    if (!currentValues) {
      TRANSFER_TYPES.forEach((transferType) => {
        transferType.activities.forEach((activity) => {
          const defaultValue = DEFAULT_TRANSFER_VALUES[transferType.id][activity.id];
          setValue(
            `functionalAssessment.transfers.${transferType.id}.${activity.id}`,
            defaultValue,
            { shouldValidate: true }
          );
        });
            ...
            {transferType.activities.map((activity) => (
              ...
              >
                ...
                <FormField
                  ...
                  defaultValue={DEFAULT_TRANSFER_VALUES[transferType.id][activity.id].level}
                  render={({ field }) => (
                    <FormItem>
                ...
                <FormField
                  ...
                  defaultValue={DEFAULT_TRANSFER_VALUES[transferType.id][activity.id].notes}
                  render={({ field }) => (
                    <FormItem>
