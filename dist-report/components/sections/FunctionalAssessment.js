...
exports.FunctionalAssessment = void 0;
const react_1 = __importDefault(require("react"));
const FunctionalAssessment = ({ assessment }) => {
    const { functionalAssessment } = assessment;
    return (<div>
      {/* Range of Motion */}
      {functionalAssessment.rangeOfMotion?.measurements?.length > 0 && (<div className="mb-6">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full table-auto">
              ...
              <tbody>
                {functionalAssessment.rangeOfMotion.measurements.map((measurement, index) => (<tr key={index} className="border-b">
      ...
      {/* Berg Balance */}
      {functionalAssessment.bergBalance?.items && (<div className="mb-6">
          <h3 className="font-semibold mb-3">Berg Balance Assessment</h3>
          <div className="grid gap-4">
            {Object.entries(functionalAssessment.bergBalance.items).map(([item, data], index) => (<div key={index} className="border-b pb-2">
            ...
            {functionalAssessment.bergBalance.totalScore && (<div className="font-bold">
      ...
      {functionalAssessment.manualMuscleTesting?.grades && (<div className="mb-6">
          <h3 className="font-semibold mb-3">Manual Muscle Testing</h3>
          {Object.entries(functionalAssessment.manualMuscleTesting.grades).map(([joint, muscles], index) => (<div key={index} className="mb-4">
              <h4 className="font-medium mb-2">{joint}</h4>
              <div className="pl-4">
                {Object.entries(muscles).map(([muscle, movements], mIndex) => (<div key={mIndex} className="mb-2">
                    <div className="font-medium">{muscle}</div>
                    <div className="pl-4">
                      {Object.entries(movements).map(([movement, sides], mvIndex) => (<div key={mvIndex}>
                          <strong>{movement}:</strong> {Object.entries(sides).map(([side, grade]) => `${side}: ${grade}`).join(', ')}
                        </div>))}
                    </div>
                  </div>))}
              </div>
...
exports.FunctionalAssessment = FunctionalAssessment;
