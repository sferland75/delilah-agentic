...
type SectionType = 'demographics' | 'injury' | 'functionalStatus' | 'adlAssessment' | 'environmental' | 'recommendations';

function getSectionPrompt(sectionType: SectionType, data: any): string {
...
async function generateReportSection(sectionType: SectionType, data: AssessmentData): Promise<ReportSection> {
...
async function generateFullReport(data: AssessmentData): Promise<string> {
    ...
    const reportParts = await Promise.all(
        sections.map(section => generateReportSection(section, data))
    );
