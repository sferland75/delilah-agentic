...
class AgentCoordinator:
    ...
    async def _setup_agents(self):
        async with get_session() as session:
            self.assessment_agent = AssessmentAgent(session)
            self.documentation_agent = DocumentationAgent(session)
            self.analysis_agent = AnalysisAgent(session)
            self.report_agent = ReportAgent(session)
            self.client_manager = ClientManager(session)
            self.therapist_manager = TherapistManager(session)
